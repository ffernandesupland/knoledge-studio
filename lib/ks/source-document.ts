...
export interface SourceAttachment { id?: string; label: string; text: string; kind?: "file" | "url"; imageId?: string; meta?: string }
...
export function orderedSources(input: SourceInput): { label: string; content: string; imageId?: string }[] {
  if (!input.content) return [
    ...(input.text.trim() ? [{ label: "pasted text", content: input.text }] : []),
    ...(input.attachments ?? []).map(a => ({ label: a.label, content: a.text, imageId: a.imageId })),
  ];
  ...
  return input.content.flatMap((b, i) => {
    ...
    return [{ label: a.label, content: a.text || `Original image: ${a.label}`, imageId: a.imageId }];
  });
}

...
export function submissionIdentity(plan: WriteOp[], snapshot: DecisionSnapshot, review?: { objectives: unknown[]; restructure: boolean; standards: string[] }): string {
  return JSON.stringify({ plan, groundContextIdentity: snapshot.groundContextIdentity, metadata: snapshot.metadata, collection: snapshot.collection, language: snapshot.language,
    ...
    reviewObjectives: review?.objectives ?? [], reviewRestructure: review?.restructure ?? false }, (_key, value) => value === false ? undefined : value);
}

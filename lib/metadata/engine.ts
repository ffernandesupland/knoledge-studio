...
import { plainEvidence } from "../ground-context/server";
...
export const RecommendationSchema = z.object({
  ...
  suggestions: z.array(z.object({ candidateId: z.string(), reason: z.string().max(600), sourceEvidence: z.string().max(500), exampleIds: z.array(z.string()).max(4), referenceEvidence: z.array(z.object({ referenceId: z.string(), quote: z.string().min(12).max(1000) })).max(8).nullable().optional() })).max(12),
  uncertainties: z.array(z.string().max(500)).max(8),
});
export function validateRecommendations(data: z.infer<typeof RecommendationSchema>, options: MetadataOption[], examples: MetadataExample[], target: WSSolution, groundContext?: GroundContextSnapshot) {
  ...
  return data.suggestions.map(s => {
    ...
    for (const evidence of s.referenceEvidence ?? []) {
      const reference = groundContext?.selection.enabled && groundContext.references.find(r => r.id === evidence.referenceId);
      if (!reference || plainEvidence(evidence.quote).length < 12 || !plainEvidence(reference.body).includes(plainEvidence(evidence.quote))) throw new Error("Metadata cited information absent from selected references.");
    }
    ...
    return { option, reason: s.reason, sourceEvidence: s.sourceEvidence, exampleIds: unique(s.exampleIds), alreadyAssigned: current.includes(option.value), referenceEvidence: s.referenceEvidence ?? [], sourceFields: (target.fields ?? []).filter(f => normalize(f.content).includes(normalize(s.sourceEvidence))).map(f => f.name) };
  });
}
...
export async function analyzeMetadataSource(target: WSSolution, ctx: RaContext, progress: (message: string) => void = () => {}, signal?: AbortSignal, groundContext?: GroundContextSnapshot): Promise<MetadataReport> {
  ...
  const examples: MetadataExample[] = neighbors.map(s => ({ id: s.id, title: s.title, summary: (s.summary ?? "").slice(0, 1200), collections: (s.collections ?? []).slice(0, 30), taxonomy: (s.taxonomy ?? []).slice(0, 30), attributes: s.attributes, attributeSet: s.attributeSetName }));
  ...
  const request = { operation: "metadataRecommend", schemaName: "metadata_recommendations", role: "Recommend metadata for a knowledge article, using catalog options and critical comparison of example articles.",
    ...
    blocks: [{ label: "article", content }, { label: "candidate options", content: JSON.stringify(options) }, { label: "published examples", content: JSON.stringify(examples) }, ...(groundContext?.selection.enabled ? referenceBlocks(groundContext) : [])], schema };
  request.task += " Selected Ground Context is reference evidence, not article scope. If a reference actually supports a suggestion, include referenceEvidence with its ID and a verbatim quote of at least 12 characters. Otherwise use an empty list. The article itself must support every classification. Never infer a product or jurisdiction from a reference alone. Attribute options are for triage, not validated assignments.";
  let result = await runOperation(request);
  account(result); check();
  let recommendations: ReturnType<typeof validateRecommendations>;
  try { recommendations = validateRecommendations(result.data, options, examples, target, groundContext); }
  catch (error) {
    ...
    account(result); check();
    recommendations = validateRecommendations(result.data, options, examples, target, groundContext);
  }

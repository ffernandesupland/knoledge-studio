...
export const PROMPT_VERSION = "2026-09-09.4";
...
export function auditAi<T>(args: RunArgs<T>, result: RunResult<T>, input: unknown) {
  const ctx = context.getStore();
  if (!ctx) return;
  db().transaction(() => {
    db().prepare(`INSERT INTO ai_calls(run_id,phase,ts,operation,model,input_tokens,output_tokens,cost_usd,prompt_version,request,response)
    ...
    db().prepare("UPDATE runs SET cost_usd=cost_usd+? WHERE id=?").run(result.costUsd, ctx.runId);
  })();
}

...
Use only catalog templates; existing update targets keep their current template. Metadata collection MUST be a nonempty catalog code, never its display name or an empty string. Language MUST be a catalog value. Choosing the destination collection is an administrative editorial decision delegated to you, not a factual claim that requires the source document to name a collection. Choose the best available destination for NEW articles based on collection labels, topic and audience, and explain the tradeoff if none is an exact match. Existing revisions preserve their parent's collection, taxonomy and language by default; the global metadata does not move them. When Discover and suggest metadata is selected, a separate research step will choose supported collections and taxonomies per resulting article after your merge decisions, honoring explicit constraints. Your global choice is the fallback for new articles when research abstains. Respect explicit metadata constraints.

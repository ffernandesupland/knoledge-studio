  ...
  return [
    ...
    n("analysis", "2 · Analyze → build the Check screen", "logic", content || o.gaps, "POST /api/run. No KB writes. Calls are recorded with model, prompt version, tokens and cost.", [
      ...
      n("dedupe", "Find duplicates", "logic", content && o.dedupe, "Retrieval finds candidates; AI judges coverage. RA relevance scores are never used as similarity percentages.", [
        n("query", "Generate a retrieval query", "ai", true, "Use distinctive technical terms from the full topic body, not just its title. One query per candidate.", undefined, "dedupeQuery"),
        n("search", "Search the RightAnswers knowledge base", "api", true, "RA Neural search, no search logging. Exclude picked source IDs; retrieve up to 8 full neighbouring articles, cached per run."),
        ...
        n("compare", "Compare full articles", "ai", o.matches !== "none", "Duplicate: same problem AND resolution. Overlapping: meaningful shared content plus unique details. Distinct: different problem/resolution despite shared words. Score 0–100 is model judgment, not a calibrated probability.", undefined, "dedupeAdjudicate"),
        n("batch", "Compare drafts within this batch", "ai", o.topics > 1, "One call over all draft pairs. Uses the same problem/resolution rubric; returns only duplicate or overlapping pairs.", undefined, "batchAdjudicate"),

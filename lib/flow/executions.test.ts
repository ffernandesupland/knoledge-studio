...
const create = async (id: string, author = "alice") => (await createRun({ id, author, path: "create", inputText: "Source", sourceIds: [], operations: [] }));
...
describe("saved executed flows", () => {
  it("stores actual prompts and mixed outcomes without inventing enabled calls or successful writes", async () => {
    (await create("mixed")); (await completeRun("mixed", { candidates: [], groups: [], costUsd: 0.1, steps: [] }));
    (await db().prepare("INSERT INTO ai_calls(run_id,phase,ts,operation,model,input_tokens,output_tokens,cost_usd,prompt_version,request,response) VALUES (?,?,?,?,?,?,?,?,?,?,?)").run("mixed", "submission", "2026-09-09", "compose", "recorded-model", 10, 20, 0.1, "old-version", JSON.stringify({ prompt: "Actual old prompt" }), JSON.stringify({ fields: [] })));
    ...
    (await freezeExecution("mixed", { runId: "mixed", user: "alice", plan, collection: "KB", language: "English", restructureEnabled: false }));
    (await saveWriteState("mixed", "mixed:0", { status: "ok", result: { kind: "create", idempotencyKey: "mixed:0", description: "Create Topic 0", outcome: "ok", solutionId: "260909000000001" } }));
    (await saveWriteState("mixed", "mixed:1", { status: "review", result: { kind: "create", idempotencyKey: "mixed:1", description: "Create Topic 1", outcome: "review", message: "Needs supported content" } }));
    const flow = (await saveExecutedFlow("mixed"));
    expect((await readExecutedFlow("mixed"))).toEqual(flow);
  ...
  it("backfills history, limits it to the authenticated owner, and keeps failed runs readable", async () => {
    (await create("private", "bob")); (await failRun("private", "Bob's private failure"));
    (await create("failed")); (await failRun("failed", "Analysis stopped"));
    ...
    expect((await readExecutedFlow("failed"))).toBeDefined();
  ...
  it("retains a completed saved tree across later reads and paginates history", async () => {
    (await create("completed")); (await markSubmitted("completed"));
    const saved = (await saveExecutedFlow("completed"));
    expect(await (await GET(request("?runId=completed"))).json()).toEqual(saved);
    for (let i = 0; i < 23; i++) { (await create(`page-${i}`)); (await failRun(`page-${i}`, "Stopped")); }
    const first = (await pastExecutions("alice")); const second = (await pastExecutions("alice", 20));

...
export async function saveExecutedFlow(runId: string): Promise<ExecutedFlow> {
  ...
  const tree: ExecutedNode[] = [
    node("sources", "1 · Sources and selected options", "input", `${run.path ?? "Content"} · ${run.candidates.length} analyzed proposals · Enabled: ${run.operations.join(", ") || "No optional operations"}`, {
      pastedCharacters: run.inputText.length, attachments: run.attachments?.map((a) => ({ label: a.label, kind: a.kind ?? "not recorded", characters: a.text.length })), sourceSolutionIds: run.sourceIds, groundContext: run.groundContext, demandSpecification: run.demandSpecification, analysisOptions: run.operations, maxNewSolutions: run.maxNewSolutions,
    }),

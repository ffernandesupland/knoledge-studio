...
export async function getConfigurationDocument(id: string, connectionId: string): Promise<ConfigurationDocument> {
  ...
  return { id: row.id, connectionId: row.connection_id, name: row.name, mime: row.mime, bytes: row.bytes, extractedText: cleanConfigurationText(row.extracted_text), createdAt: row.created_at };
}
...
export async function storeConfigurationDocument(args: { connectionId: string; createdBy: string; name: string; mime: string; bytes: Buffer }): Promise<ConfigurationDocument> {
  ...
  const extractedText = cleanConfigurationText(parsed.text);
  const id = randomUUID();
  ...
  await db().transaction(async () => {
    await db().prepare("INSERT INTO configuration_documents(id,connection_id,name,mime,bytes,extracted_text,created_by,created_at) VALUES(?,?,?,?,?,?,?,?)")
      .run(id, args.connectionId, parsed.name, args.mime || "text/plain", args.bytes.length, extractedText, args.createdBy, createdAt);
  ...
  return { id, connectionId: args.connectionId, name: parsed.name, mime: args.mime || "text/plain", bytes: args.bytes.length, extractedText, createdAt };
}

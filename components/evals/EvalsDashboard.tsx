...
export function EvalsDashboard() {
  ...
  return <main className="ks-scroll" style={{ maxWidth: 1320, margin: "0 auto", padding: "34px 28px 56px" }}>
    ...
    <p style={{ maxWidth: 800 }}>Compare fixed rubrics and draft scores across the pipeline scenarios you have evaluated. Evaluations are observational only and never change draft readiness or publishing behavior.</p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 14, margin: "24px 0" }}>
      <section className="ks-card" style={{ padding: 18 }}><span className="ks-eyebrow">SCENARIOS</span><strong style={{ display: "block", fontSize: 28, marginTop: 6 }}>{rubrics.length}</strong><small>Fixed pipeline signatures</small></section>
      <section className="ks-card" style={{ padding: 18 }}><span className="ks-eyebrow">EVALUATIONS</span><strong style={{ display: "block", fontSize: 28, marginTop: 6 }}>{total}</strong><small>Prepared drafts scored</small></section>
    ...
    <section className="ks-card" style={{ overflowX: "auto", padding: 0 }}>
      <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 920 }}><thead><tr><th>Scenario</th><th>Pipeline</th><th>Rubric</th><th>Evaluations</th><th>Average score</th><th>Latest evaluation</th><th aria-label="Actions" /></tr></thead><tbody>
        {rubrics.map(rubric => <tr key={rubric.id}><td><strong>{rubric.scenarioLabel}</strong><small style={{ display: "block", marginTop: 5 }}>Revision {rubric.revision} · {rubric.rubric.criteria.length} criteria</small></td><td>{rubric.pipeline.template.name}<small style={{ display: "block", marginTop: 5 }}>{rubric.pipeline.context.ground ? "Grounded" : "Not grounded"} · {rubric.pipeline.context.standards ? "Standards" : "No standards"}</small></td><td>{rubric.compilerModel}<small style={{ display: "block", marginTop: 5 }}>Prompt {rubric.promptVersion}</small></td><td>{rubric.evaluationCount}</td><td><strong>{score(rubric.averageScore)}</strong></td><td>{when(rubric.latestResult?.createdAt)}<small style={{ display: "block", marginTop: 5 }}>{rubric.latestResult?.judgeModel ?? "—"}</small></td><td><button type="button" className="ds-btn ds-btn-secondary" onClick={() => setSelected(rubric)}>View rubric</button></td></tr>)}
        {!loading && !rubrics.length && <tr><td colSpan={7} style={{ padding: 28 }}>No evaluations yet. Prepare a draft in Knowledge Studio, then select <strong>Run evaluation</strong> from the draft inspector.</td></tr>}
...
function RubricDialog({ rubric, onClose }: { rubric: EvalDashboardRubric; onClose: () => void }) {
  return <div className="entity-modal-scrim" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="entity-modal" role="dialog" aria-modal="true" aria-labelledby="eval-rubric-title" style={{ maxWidth: 900, maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
    <div className="entity-modal-hdr"><div className="entity-modal-title"><span className="ms" aria-hidden="true">analytics</span><div><span className="ks-eyebrow">FIXED RUBRIC</span><h2 id="eval-rubric-title">{rubric.rubric.title}</h2><p>{rubric.rubric.summary}</p></div></div><button type="button" className="ds-btn ds-btn-secondary" onClick={onClose}>Close</button></div>
    <div className="entity-modal-body" style={{ overflowY: "auto", flex: 1 }}><p><strong>{rubric.scenarioLabel}</strong> · Revision {rubric.revision} · {rubric.evaluationCount} saved evaluation{rubric.evaluationCount === 1 ? "" : "s"}</p><div style={{ display: "grid", gap: 10 }}>{rubric.rubric.criteria.map(criterion => <article className="ks-card" key={criterion.id} style={{ padding: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}><h3 style={{ margin: 0 }}>{criterion.title}</h3><span className="ks-chip">Weight {criterion.weight}</span></div><p style={{ marginBottom: 6 }}>{criterion.description}</p><small><strong>Evidence:</strong> {criterion.evidenceRequired}</small></article>)}</div></div>

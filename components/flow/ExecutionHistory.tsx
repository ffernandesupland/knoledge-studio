  ...
  return <section className="flow-history">
    ...
    {runId && <>
      ...
      {flow && <>
        ...
        {flow.mode === "autonomous" && <><p><Link href={`/?autonomousRun=${encodeURIComponent(flow.runId)}`}>Open autonomous run status</Link></p><AutonomousLog key={flow.runId} runId={flow.runId} live={!["submitted", "partial", "error"].includes(flow.status)} /></>}

...
export async function GET(request: Request) {
  try {
    ...
    const run = getRun(new URL(request.url).searchParams.get("runId") ?? "");
    assertOwner(run, user);
    const calls = db().prepare(`SELECT id, phase, ts, operation, model, input_tokens AS inputTokens,
    ...
    return Response.json({ runId: run.id, status: run.status, costUsd: run.costUsd, steps: run.steps, calls, results: executionResults(run.id) }, { headers: { "cache-control": "no-store" } });

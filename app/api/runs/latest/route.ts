...
export async function GET(request: Request) {
  try {
    const run = getResumableRun(await requireActor(request));
    return Response.json({ run, execution: run ? loadExecution<ExecuteArgs>(run.id) : undefined, results: run ? executionResults(run.id) : [] });
  } catch (e) { return apiError(e); }
}
export async function DELETE(request: Request) {
  try { discardResumableRun(await requireActor(request)); return Response.json({ ok: true }); }
  catch (e) { return apiError(e); }
}
export async function PATCH(request: Request) {
  try {
    ...
    if (loadExecution(run.id)) return Response.json({ ok: true, frozen: true });
    saveSnapshot(run.id, canonicalSnapshot(run, body.snapshot));

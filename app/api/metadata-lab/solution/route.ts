...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      ...
      const [solution, collections] = await Promise.all([ra.getSolution(id, { impUser: actor, connection }), ra.getCollections({ impUser: actor, connection })]);
    ...
    const result = await ra.search({ queryText: q, page, searchType: "Hybrid", verboseResult: true, verboseResultFields: "title,summary,status", loggingEnabled: false }, { impUser: actor, connection });
